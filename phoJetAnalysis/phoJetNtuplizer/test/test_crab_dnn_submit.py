...
config = config()
name = 'MC2018_Autumn18_monoHiggs_04Apr_test'
...
for sample in listOfSamples:  
  os.popen('cp run_dnn_test_v2.py run_dnn_'+sample+'.py')
  os.popen('cp myscript.sh myscript_'+sample+'.sh')
  
  with open('myscript_'+sample+'.sh') as oldFile:
    newText = oldFile.read().replace('run_dnn_test_v2.py', 'run_dnn_'+sample+'.py')
