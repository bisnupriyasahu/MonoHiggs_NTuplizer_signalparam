...
vector<float>  tau_byRawDeepTau2017v2p1VSjet_;
// tau DNN-based ID
...
//  tree->Branch("tau_LeadChargedHadronExists",                           &tau_LeadChargedHadronExists_);
  ...
  tree->Branch("tau_byRawDeepTau2017v2p1VSjet", &tau_byRawDeepTau2017v2p1VSjet_);
...
void phoJetNtuplizer::fillTaus (const edm::Event& iEvent){
  ...
  for(vector<pat::Tau>::const_iterator itau = tauHandle->begin(); itau != tauHandle->end(); ++itau) {
    ...
    std::cout<<itau->signalChargedHadrCands().size() << "  " <<std::endl;
    //std::cout<< itau-> <<endl;
    ...
    // tau DNN-based ID
    tau_byRawDeepTau2017v2p1VSjet_                   .push_back(itau->tauID("byDeepTau2017v2p1VSjetraw"));
...
void phoJetNtuplizer::initTaus(){
  ...
  // tau DNN-based ID
  tau_byRawDeepTau2017v2p1VSjet_.clear();
