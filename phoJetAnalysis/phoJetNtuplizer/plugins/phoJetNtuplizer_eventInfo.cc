...
bool          DoubleMediumTau40TightID_;
bool          DoubleTightTau40_;
bool          DoubleTightTau35TightID_;
bool          DoubleTightTauHPS40_;
...
ULong64_t     HLTTauPath_;
ULong64_t     HLTTauPathIsPrescaled_;
ULong64_t     HLTTauPathRejectedByPS_;
...
void phoJetNtuplizer::branchEventInfo(TTree* tree) {
  ...
  tree->Branch("DoubleMediumTau40TightID", &DoubleMediumTau40TightID_ );
  tree->Branch("DoubleTightTau40", &DoubleTightTau40_ );
  tree->Branch("DoubleTightTau35TightID", &DoubleTightTau35TightID_ );
  tree->Branch("DoubleTightTauHPS40", &DoubleTightTauHPS40_ );
  tree->Branch("HLTTauPath",               &HLTTauPath_);
  tree->Branch("HLTTauPathIsPrescaled",    &HLTTauPathIsPrescaled_);
  tree->Branch("HLTTauPathRejectedByPS",   &HLTTauPathRejectedByPS_);
  tree->Branch("tau_HLTPath",              &tau_HLTPath_);
}
...
void phoJetNtuplizer::fillEventInfo(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  for (size_t i = 0; i < trgNames.size(); ++i) {
    ...
    if(i==0){
      DoubleMediumTau40TightID_ = false;
      DoubleTightTau40_ = false;
      DoubleTightTau35TightID_ = false;
      DoubleTightTauHPS40_ = false;
    }
    if(isFired){
      if(name.find("HLT_DoubleMediumChargedIsoPFTau40_Trk1_TightID_eta2p1_Reg_v")     != string::npos) {DoubleMediumTau40TightID_ = true; tau_HLTPath_.push_back(1); } 
      if(name.find("HLT_DoubleTightChargedIsoPFTau40_Trk1_eta2p1_Reg_v")              != string::npos) {DoubleTightTau40_         = true; tau_HLTPath_.push_back(2); }
      if(name.find("HLT_DoubleTightChargedIsoPFTau35_Trk1_TightID_eta2p1_Reg_v")      != string::npos) {DoubleTightTau35TightID_  = true; tau_HLTPath_.push_back(3); }
      if(name.find("HLT_DoubleTightChargedIsoPFTauHPS40_Trk1_eta2p1_Reg_v")           != string::npos) {DoubleTightTauHPS40_      = true; tau_HLTPath_.push_back(4); }
    }
    int bitTau2018 = -1;
    if(name.find("HLT_DoubleMediumChargedIsoPFTau40_Trk1_TightID_eta2p1_Reg_v")     != string::npos) bitTau2018 = 1;
    if(name.find("HLT_DoubleTightChargedIsoPFTau40_Trk1_eta2p1_Reg_v")              != string::npos) bitTau2018 = 2;
    if(name.find("HLT_DoubleTightChargedIsoPFTau35_Trk1_TightID_eta2p1_Reg_v")      != string::npos) bitTau2018 = 3;
    if(name.find("HLT_DoubleTightChargedIsoPFTauHPS40_Trk1_eta2p1_Reg_v")           != string::npos) bitTau2018 = 4;
    ...
    if (bitTau2018 > 0) {                                                                                                
      HLTTauPath_             |= (isFired << bitTau2018);
      HLTTauPathIsPrescaled_  |= (isPrescaled << bitTau2018);
      HLTTauPathRejectedByPS_ |= (isRejectedByHLTPS << bitTau2018);
    }
...
void phoJetNtuplizer::initEventInfo(){
  ...
  DoubleTightTauHPS40_ = false;
  HLTTauPath_               = 0;
  HLTTauPathIsPrescaled_    = 0;
  HLTTauPathRejectedByPS_   = 0;
  tau_HLTPath_.clear();
